...
function setCPUResources()
{
   ...
   #SED "s#^MULTIPROCESSOR.*#MULTIPROCESSOR $E_MULTI#g" "${ONCONFIG_PATH}"
   ...
   then
      SED "s#^LOCKS.*#LOCKS 50000#g" "${ONCONFIG_PATH}"
      SED "s#^LOGBUFF.*#LOGBUFF 128#g" "${ONCONFIG_PATH}"
      SED "s#^NETTYPE.*#NETTYPE soctcp,1,200,CPU#g" "${ONCONFIG_PATH}"
      SED "s#^PHYSBUFF.*#PHYSBUFF 128#g" "${ONCONFIG_PATH}"
      SED "s#^VP_MEMORY_CACHE_KB.*#VP_MEMORY_CACHE_KB 0#g" "${ONCONFIG_PATH}"

   ### Medium System
   ### Relate to informix_config.medium
   elif [ `expr $E_NUMCPU` -lt 8 ]
   then
      SED "s#^LOCKS.*#LOCKS 100000#g" "${ONCONFIG_PATH}"
      SED "s#^LOGBUFF.*#LOGBUFF 256#g" "${ONCONFIG_PATH}"
      SED "s#^NETTYPE.*#NETTYPE soctcp,4,200,CPU#g" "${ONCONFIG_PATH}"
      SED "s#^PHYSBUFF.*#PHYSBUFF 256#g" "${ONCONFIG_PATH}"
      SED "s#^VP_MEMORY_CACHE_KB.*#VP_MEMORY_CACHE_KB 2048#g" "${ONCONFIG_PATH}"

   ### Large System
   ### Relate to informix_config.large
   else
      SED "s#^LOCKS.*#LOCKS 250000#g" "${ONCONFIG_PATH}"
      SED "s#^LOGBUFF.*#LOGBUFF 512#g" "${ONCONFIG_PATH}"
      SED "s#^NETTYPE.*#NETTYPE soctcp,8,200,CPU#g" "${ONCONFIG_PATH}"
      SED "s#^PHYSBUFF.*#PHYSBUFF 512#g" "${ONCONFIG_PATH}"
      SED "s#^VP_MEMORY_CACHE_KB.*#VP_MEMORY_CACHE_KB 4096#g" "${ONCONFIG_PATH}"
...
function setMEMResources()
{
   ...
   SED "s#^BUFFERPOOL size=2k.*#BUFFERPOOL size=2k,buffers=$vBUFFERS,lrus=8,lru_min_dirty=50,lru_max_dirty=60#g" "${ONCONFIG_PATH}"
   SED "s#^SHMTOTAL.*#SHMTOTAL $vSHMTOTAL#g" "${ONCONFIG_PATH}"
   SED "s#^SHMVIRTSIZE.*#SHMVIRTSIZE $vSHMVIRTSIZE#g" "${ONCONFIG_PATH}"
   SED "s#^DS_NONPDQ_QUERY_MEM.*#DS_NONPDQ_QUERY_MEM $vNONPDQ#g" "${ONCONFIG_PATH}"
...
### RESIDENT -1
   ...
   SED "s#^AUTO_TUNE.*#AUTO_TUNE 1#g" "${ONCONFIG_PATH}"
   SED "s#^DIRECT_IO.*#DIRECT_IO 1#g" "${ONCONFIG_PATH}"
   SED "s#^DUMPSHMEM.*#DUMPSHMEM 0#g" "${ONCONFIG_PATH}"
   SED "s#^RESIDENT.*#RESIDENT -1#g" "${ONCONFIG_PATH}"
